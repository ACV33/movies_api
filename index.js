...
// WORKING: auth
// NOT WORKING: actual user retrieval
// See change for fix
app.get(
  "/users/:username",
...
// NOT working, see potential fixes
// make sure to follow the schema in models.js
app.put(
  "/users/:Name",
  [
    check("Username", "Username is required").isLength({ min: 5 }),
...
// POST
app.post(
  "/users/:Name/favoriteMovies/:Movie",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    Users.find(
      { Username: req.params.Name },
...
// DELETE
// NOT working: returns 200, but we don't actually delete the user
app.delete(
  "/users/:Username",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    // https://mongoosejs.com/docs/api.html#query_Query-deleteOne
    // add code here to delete the user, ie:
